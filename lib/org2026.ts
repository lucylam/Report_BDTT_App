...
export const ORG_GROUP_NAMES = Object.values(ORG_GROUPS);

export const ORG_ROLE_VALUES: readonly OrgRole[] = [
  "toTruong",
  "nhomTruong",
  "nhomPho",
  "pnt",
...
const subgroupAssignments: Record<string, string> = {
  ...
  [groupKey(ORG_GROUPS.chapHanh, "PN11")]: "Phụ trách hạng mục TB Chấp hành PN11",
  [groupKey(ORG_GROUPS.chapHanh, "PN12")]: "Phụ trách hạng mục TB Chấp hành PN12",
...
export const getOrgTitle = (
  ...
  if (orgRole === "nhomTruong" || orgRole === "nhomPho") {
    return `${getOrgRoleLabel(orgRole)} - ${groupLabel(orgGroup)}`;
  }
  if (subgroup) return `${getOrgRoleLabel(orgRole)} - ${subgroupLabel(subgroup)} - ${groupLabel(orgGroup)}`;
  return `${getOrgRoleLabel(orgRole)} - ${groupLabel(orgGroup)}`;
};

export const getOrgAssignment = (
  username: string,
  orgRole: OrgRole,
  orgGroup: string,
  subgroup: string,
  usePersonalAssignment = true
): string => {
  const personalAssignment = usePersonalAssignment ? personalAssignments[username] : undefined;
...
const allGroupNames = ORG_GROUP_NAMES;
...
export const ORG_2026_SEEDS: readonly AccountSeed2026[] = [
  ...
  person("Trần Văn Hậu", "HAUTV", "hautv@pvcfc.com.vn", "admin", "nhomPho", ORG_GROUPS.chapHanh, "", nhomPhoGroups(ORG_GROUPS.chapHanh), []),
  ...
  person("Nguyễn Văn Bình", "000333", "binhnv1@pvcfc.com.vn", "admin", "pnt", ORG_GROUPS.chapHanh, "PN3", [], [groupKey(ORG_GROUPS.chapHanh, "PN3")]),
  person("Nguyễn Ngọc Sơn", "000331", "sonnn@pvcfc.com.vn", "admin", "pnt", ORG_GROUPS.chapHanh, "PN4", [], [groupKey(ORG_GROUPS.chapHanh, "PN4")]),
  person("Ngô Thanh Lâm", "001416", "lamnt@pvcfc.com.vn", "admin", "pnt", ORG_GROUPS.chapHanh, "PN5", [], [groupKey(ORG_GROUPS.chapHanh, "PN5")]),
  person("Trịnh Thạnh Lợi", "000103", "loitt@pvcfc.com.vn", "admin", "pnt", ORG_GROUPS.chapHanh, "PN6", [], [groupKey(ORG_GROUPS.chapHanh, "PN6")]),
  person("Nguyễn Văn Vũ", "000172", "vunv@pvcfc.com.vn", "admin", "pnt", ORG_GROUPS.chapHanh, "PN7", [], [groupKey(ORG_GROUPS.chapHanh, "PN7")]),
  ...
  person("Dương Văn Hằng", "000469", "hangdv@pvcfc.com.vn", "admin", "pnt", ORG_GROUPS.chapHanh, "PN10", [], [groupKey(ORG_GROUPS.chapHanh, "PN10")]),
  person("Hồ Đức Trung", "001088", "trunghd@pvcfc.com.vn", "admin", "pnt", ORG_GROUPS.chapHanh, "PN11", [], [groupKey(ORG_GROUPS.chapHanh, "PN11")]),
...
export const getOrgSubgroups = (orgGroup: string): string[] =>
  Array.from(
    new Set(
      ORG_2026_SEEDS.filter((profile) => profile.orgGroup === orgGroup)
        .map((profile) => profile.subgroup)
        .filter(Boolean)
    )
  ).sort((left, right) => left.localeCompare(right, "vi", { numeric: true }));

export const getUserRoleForOrgRole = (orgRole: OrgRole): UserRole =>
  orgRole === "member" || orgRole === "placeholder" ? "worker" : "admin";

export const deriveOrgMetadata = (
  username: string,
  orgRole: OrgRole,
  orgGroup: string,
  subgroup: string,
  usePersonalAssignment = false
): Pick<
  OrgMetadata,
...
> => {
  ...
  return {
    orgTitle: getOrgTitle(orgRole, orgGroup, subgroup),
    orgAssignment: getOrgAssignment(
      username,
      orgRole,
      orgGroup,
      subgroup,
      usePersonalAssignment
    ),
    managedGroups,
    managedSubgroups
  };
